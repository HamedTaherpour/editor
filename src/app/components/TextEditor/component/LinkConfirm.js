...
const LinkConfirm = ({ onBtnSetLinkClick }) => {
  ...
  return (
    <div className="flex flex-row p-3 rounded-xl border border-gray-2 bg-white w-80 shadow-md gap-x-1.5">
      ...
      <input placeholder="لینک را اینجا بنویسید." onChange={(e) => setLink(e.target.value)} className="placeholder:text-gray-5 text-sm px-3 py-1 rounded-lg border border-gray-2 w-full outline-none" />
    </div>
  );
};
