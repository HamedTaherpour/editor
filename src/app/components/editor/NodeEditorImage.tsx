...
const NodeEditorImage = (props: Props) => {
  ...
  return (
    <div ref={rootRef} className="w-full rounded flex flex-col">
      ...
      <div className={showFileSelected ? "hidden" : null + " w-full flex justify-center"}>
            ...
            <div className="absolute w-full h-full inset-0 flex flex-col justify-between p-2 opacity-0 hover:opacity-100 app-base-transform">
              <div className="flex flex-row justify-start items-start h-2/6">
                <div className="bg-black/80 rounded-lg px-1.5 flex flex-row items-center gap-x-2">
                  <AppTooltip
                    activator={
                      <button onClick={onBtnAddCaptionClick} className="w-6 h-7 text-white">
                        C
                      </button>
                    }
      ...
      {!showFileSelected && node.path.length <= 0 ? (
        <span onClick={() => setShowFileSelected(true)} className="text-xs text-gray-7">
          یک عکس انتخاب کنید
        </span>
      ) : null}
    </div>
